...
2. Call `begin_upload` with `filename`, `mime`, `size_bytes`. It returns `s3_url`, `upload_id`, and `put_example` (a curl one-liner). Note: this SKU is more expensive than `upload_image` (typically $0.20 vs $0.10) — make sure `CLOUDUP_MAX_USD` is set high enough or the call will fail with `mpp-remote: charge … exceeds MPP_MAX_AMOUNT_USD=…`.
...
Each upload is paid for by the user's wallet, provisioned via `/cloudup-setup` (stored in macOS Keychain) or as a back-compat fallback the `CLOUDUP_WALLET_KEY` env var. The default cap is $0.10 per call (`CLOUDUP_MAX_USD`). Path A (`upload_image` / `quick_upload`) fits under the default cap; Path B (`begin_upload`) typically charges $0.20, so users who need it must raise the cap to at least $0.25.
